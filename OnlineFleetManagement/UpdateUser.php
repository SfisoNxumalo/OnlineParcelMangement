...
$phone = Filter_input(INPUT_POST, 'ucontact');
$address = Filter_input(INPUT_POST, 'uaddress');
$worksat = Filter_input(INPUT_POST, 'worksat');
$cAddress = Filter_input(INPUT_POST, 'caddress');
$username = Filter_input(INPUT_POST, 'username');
$Password = Filter_input(INPUT_POST, 'password');

if(!empty($name) || !empty($email)|| !empty($phone) || !empty($address) || !empty($worksat) || !empty($cAddress) || !empty($username) || !empty($Password))
{	
    $pID = $_GET['id'];
		$host = "localhost";
		$dbusername = "root";
		...
		{ 
			$sql = "UPDATE tblusers SET NAME = '$name', EMAIL = '$email', CONTACT = '$phone', ADDRESS = '$address', WORKS_AT = '$worksat', COMPANY_ADDRESS = '$cAddress', USERNAME = '$username', PASSWORD = '$Password' WHERE ID = '$pID'";		//SQL code that writes data into the database.

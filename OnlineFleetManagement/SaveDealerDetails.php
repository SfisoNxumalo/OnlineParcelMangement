    ...
    session_start();
                ...
                if(isset($_SESSION['Lusername']) )
                {
                    ...
                    if($_SESSION['Lcompany'] == "Main" || $_SESSION['Lcompany'] == "Different Branch")
                    {
                        ...
                        { 
                            $_SESSION['Lcompany'];
                                $sql = "INSERT INTO tbldealer (NAME, ADDRESS, EMAIL, CONTACT, PROVINCE) values('$DealerName', '$DealerAddress', '$DealerEmail', '$DealeContact', '$DealerProvince')";		//SQL code that writes data into the database.

                                if($conn->query($sql))		//when information is added to the databse successfully the messgage below is what the user will see.
                                {
                                   echo '<script>alert("New Dealer saved")</script>';
